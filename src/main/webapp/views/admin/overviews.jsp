...
<title>AccountManager | Video</title>
...
<body class="hold-transition sidebar-mini layout-fixed">
		...
		<aside class="main-sidebar sidebar-dark-primary elevation-4">
			...
			<div class="sidebar">
				...
				<div class="user-panel mt-3 pb-3 mb-3 d-flex">
					<div class="info">
						<a href="#" class="d-block">Hi,${sessionScope.currentUser.username}</a>
					</div>
				</div>
							...
							<tbody>
								<c:forEach items="${videos}" var="item">
									<tr>
										...
										<td><img src="${item.poster}" width="230px"
											height="150px"></td>
										<td>
											<a href="<c:url value='/admin/video?action=edit&href=${item.href}'/>" type="button" class="btn bg-gradient-info btn-md">Edit</a>
											<button type="button" onclick="deleteVideo('${item.href}')"
												class="btn bg-gradient-info btn-md">Delete</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
	...
	<script>
		function deleteVideo(href) {
			$.ajax({
				url:'/AsmFinal/admin/video?action=delete&href=' + href
			}).then(function(data) {
				window.location.href = "http://localhost:8080/AsmFinal/admin/video?action=view";
			}).fail(function(error) {
				alert("Fail");
			})
		}
